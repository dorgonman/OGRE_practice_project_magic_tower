...
{
	button *b = new button(name, material, position, width, height, mButtonMgr, NULL, this, scheme, isActiveButton);

	panel->addChildImpl(b->panel);

	childButtons.push_back(b);
	mButtonMgr->buttons.push_back(b);  //list for all buttons.
	return b;
}
...
{
	return createTextInputArea(textInputName, a, mButtonMgr->defaultTextFieldMaterial, position, width, height, textPosX, textPosY,  textStyle, isActiveButton);  //use the last textStyle assigned to this button and pass it on.
}
...
{
	textInputArea *t = new textInputArea(mButtonMgr, textInputName, a, material, position, this, width, height,  textPosX,  textPosY, style, isActiveButton);
	t->setMovable(false);

	panel->addChildImpl(t->panel);

	childButtons.push_back(t);
	mButtonMgr->buttons.push_back(t);  //list for all buttons( and textInputAreas)
	return t;  
}
...
{
	buttonMesh * bm = new buttonMesh(name, meshFile, mButtonMgr, this, mButtonMgr->zOrderCounter,  positionX,  positionY, width,height);
	mButtonMgr->zOrderCounter++;

	buttonMeshes.push_back(bm);
	mButtonMgr->buttonMeshes.push_back(bm);
	return bm;
}
...
{	
	...
	{
		int winWidth = mButtonMgr->screenResX;
		int winHeight = mButtonMgr->screenResY;
...
{
	setParent(mButtonMgr->getButton(newParentName));
}
...
//			setOpacity(1);//clear any half transparencies from past events.
	...
	if (rotateMB)//send the message to children buttonMeshes
	{
		for ( meshItr = buttonMeshes.begin(); meshItr != buttonMeshes.end(); ++meshItr )
		{	
			(*meshItr)->injectMouseDown(mButtonMgr->mouseX, mButtonMgr->mouseY);
		}
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    **   BUTTONMESH CLASS    **   ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
buttonMesh::buttonMesh( const Ogre::String name, const Ogre::String meshFile, buttonManager * mButtonMgr, button* parentButton, short unsigned int zOrder,  short positionX, short positionY,  short width, short height) :
name(name),
mButtonMgr(mButtonMgr),
parent(parentButton),
relX(positionX),
relY(positionY),
zoom(1),	
...
turnSpeedModifier(.025) //less makes it turn slower
{
	...
	baseNode = new Ogre::SceneNode(mButtonMgr->sceneMgr, name +"_buttonGUIBaseNode");  //because this node is going into an overlay, should not use sceneManager to make this instance.  See comments in ogreoverlay.h for more info.
	e = mButtonMgr->sceneMgr->createEntity(name + "_buttonGUIEntity", meshFile);
...
buttonMesh::~buttonMesh()
{
	...
	if (baseNode)
	{
		if (e)
		{
			...
			mButtonMgr->sceneMgr->destroyEntity(e);
		}
		delete baseNode;
		baseNode = NULL;
	}
...
buttonMesh * buttonMesh::setDimensions(short w, short h)
{
	width = w;  
	height = h;
	Ogre::Radian fov = mButtonMgr->sceneMgr->getCamera(mButtonMgr->camera)->getFOVy();
	...
	aspectRatio = float(mButtonMgr->screenResX)/float(mButtonMgr->screenResY);
	screenSpaceModifier = (float(1)/float(mButtonMgr->screenResY)) ;
	...
	if((rotatableX)||(rotatableY))
	{
		if(rotatableX)
		{
			if (momentumX)
			{
				...
				momentumX = momentumX  - friction * (float)mButtonMgr->timeDiff;
				if (momentumX <= 0)
					momentumX = 0;
			}
		}
		if(rotatableY)
		{
			if (momentumY)
			{
				...
				momentumY = momentumY - friction * (float)mButtonMgr->timeDiff;
				if (momentumY <= 0)
					momentumY = 0;
			}
		}
...
void buttonManager::showAllButton(){
	for ( buttonItr = buttons.begin(); buttonItr != buttons.end(); ++buttonItr ){
		buttonGUI::button* b = *buttonItr;
		b->show(false);
		b->setActive(true);
	}
}
...
void buttonManager::hideAllButton(){
	for ( buttonItr = buttons.begin(); buttonItr != buttons.end(); ++buttonItr ){
