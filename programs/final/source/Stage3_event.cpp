...
void Stage3::setEvent10(){
	...
	for(int i = 0; i < 5; i++){
		...
		if(weapon){
			...
			weapon->setCoolDown(1);
			weapon->setBulletSize(Ogre::Vector3(10, 10, 10));
		}
	}
	for(int i = 5; i < 10; i++){
		String guns;
...
void Stage3::doEvent10(){
	...
	if(weapon){
			...
			if(p->isAlive()){
				...
				if((int)mTime % 2 == 0){
					targetPosition.x = rand_number(0, 5000);
					targetPosition.y = rand_number(0, 3000);
					targetPosition.z = rand_number(0, 5000);
					mTime++;
				}
				mAIEng->runTo(p, targetPosition);
